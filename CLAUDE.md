...
- Before merging, ask Claude Code directly to check both reviews and triage findings by severity: read `CODEX_CODE_REVIEW.md` (already local from the pre-push hook) and fetch the Claude review comment (`gh pr view <N> --comments`). Fix P1s immediately and push again. For P2/P3s, don't iterate — file each as a GitHub Issue and move on; a fresh push re-runs the review and can surface new lower-severity findings indefinitely, so chasing every one blocks the PR without bound. Backlog issues get folded into a future phase, not resolved inline mid-PR. Neither review can loop itself in — this only happens when you ask.
