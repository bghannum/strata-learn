...
- [x] `claude-review.yml`'s `--allowedTools` was scoped to just the posting tools (`gh pr comment`, `gh pr diff`, `gh pr view`, inline comments) — worked fine on a small docs-only PR, but on a real multi-file code diff Claude wanted to read full file contents for context, not just diff hunks, and every attempt got denied (found on PR #5, Phase 1.5, 2026-08-11).
      Confirmed via the run log: 6+ minutes, 27 turns, 43 permission denials, zero comments posted, job still green throughout — another silent-failure mode for this workflow, distinct from the earlier `post_as_review` one. Resolved by adding `Read`, `Grep`, `Glob`, and read-only `git diff`/`log`/`show` to the allowed list. Per the workflow-file self-verification gotcha (see `CLAUDE.md`), this fix can't be confirmed from its own PR — verify on whichever PR merges next.
